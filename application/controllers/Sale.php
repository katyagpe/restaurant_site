...
class Sale extends CI_Controller {
    ...
    public function update_order_status_ajax()
    {
        ...
        $payCash = $this->input->post('payCash');
        $payOne = $this->input->post('payOne');
        $payTwo = $this->input->post('payTwo');
        $payThree = $this->input->post('payThree');
        ...
        if($close_order=='true'){
            ...
            if($is_just_cloase){
                ...
                $order_status = array('paid_amount' =>  $paid_amount, 'due_amount' => $due_amount, 'order_status' => 3,'payment_method_id'=>$payment_method_type,'close_time'=>date('H:i:s'), 'pay_cash' =>$payCash, 'pay_one' =>$payOne, 'pay_two' =>$payTwo, 'pay_three' =>$payThree);    
            }
            
        }else{
            $order_status = array('paid_amount' => $paid_amount,'due_amount' => $due_amount,'order_status' => 2,'payment_method_id'=>$payment_method_type, 'pay_cash' =>$payCash, 'pay_one' =>$payOne, 'pay_two' =>$payTwo, 'pay_three' =>$payThree);
        }
