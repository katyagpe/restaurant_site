...
/*	$(document).on('mouseleave','.header_part_left_left .outlet_holder .outlet_holder_moving',function(){
	...
	/* cantidades de pago mixto */
	/*$("#amount_payment_cash").on('keyup', function(e){
		var given_cash = ($(this).val()!="")?$(this).val():0;
	});
	$("#amount_payment_one").on('keyup', function(e){
		var given_payment_one = ($(this).val()!="")?$(this).val():0;
	});
	$("#amount_payment_two").on('keyup', function(e){
		var given_payment_two = ($(this).val()!="")?$(this).val():0;
	});
	$("#amount_payment_three").on('keyup', function(e){
		var given_payment_three = ($(this).val()!="")?$(this).val():0;
	});*/
	/* cantidades de pago mixto */

	$(document).on('keyup', '.amount_payment_mix', function(){
		summix();
	});
	
	function summix() {
		var total = 0;
		$(".amount_payment_mix").each(function (){
		...
		$('#total_amount_payMethod').val(total_fixed);
		
		
		$('#pay_amount_invoice_input').val(total_fixed);
		
		var amountTotal = $('#finalize_total_payable').text();
		var restAmount = amountTotal - total_fixed ;
		$('#due_amount_invoice_input').val(restAmount);
	}
...
/*	$(document).on('mouseenter','.header_part_left_left .outlet_holder .outlet_holder_moving',function(){
  ...
  //           })
	...
	$("#finalie_order_payment_method").on('change', function(event){
		var typePayment = $("#finalie_order_payment_method option:selected").val();
		if( typePayment == 6 ) {
			$('#methodPay')[0].style.display = "block";
		} else {
			$('#methodPay')[0].style.display = "none";
		}
	});
	...
	$('#finalize_order_button').on('click',function(){
		...
		if($('.holder .order_details > .single_order[data-selected=selected]').length>0){
			/* Pago mixto */
			var payCash   = $('#amount_payment_cash').val();
			var payOne    = $('#amount_payment_one').val();
			var payTwo    = $('#amount_payment_two').val();
			var payThree  = $('#amount_payment_three').val();
			...
			print_invoice_and_close(sale_id,payment_method_type,invoice_create_type,paid_amount,due_amount, payCash, payOne, payTwo, payThree);
			reset_finalize_modal();
		}else{
			swal({
                title: warning+"!",
...
function print_invoice_and_close(sale_id,payment_method_type,invoice_create_type,paid_amount,due_amount, payCash, payOne, payTwo, payThree){
	if(invoice_create_type == 1){ //if type is 1 then update order status to invoiced order, and update payment method type
		update_order_status_to_invoiced(sale_id,payment_method_type,paid_amount,due_amount, payCash, payOne, payTwo, payThree);
	}else if(invoice_create_type == 2){ //then change order status to close, close time update, payment method type update, 
		close_order(sale_id,payment_method_type,paid_amount,due_amount, payCash, payOne, payTwo, payThree);
	}
	print_invoice(sale_id);
}
...
function update_order_status_to_invoiced(sale_id,payment_method_type,paid_amount,due_amount, payCash, payOne, payTwo, payThree){
	$.ajax({
